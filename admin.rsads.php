<?php
/**
 * @version 1.5.0 $Id: admin.rsads.php
...
function display() {
	...
	$database->setQuery("select count(*) as tot from `#__".RSWEBSOLS_TABLE_PREFIX."_ad` where 1");
	$cnt = $database->loadObject();
	$total_page = ceil($cnt->tot/$limit2);
	
	$database->setQuery("select * from `#__".RSWEBSOLS_TABLE_PREFIX."_ad` where 1 order by `id` desc limit ".$limit1.",".$limit2."");
	$items = $database->loadObjectList();
?>
<div>
<div>&nbsp;</div>
<table width="100%" cellpadding="0" cellspacing="0"><tr><td><h1>Manage Ads</h1></td><td align="right"><a href="index.php?option=<?php echo $_REQUEST['option']; ?>&task=<?php echo $_REQUEST['task']; ?>&action=add&page=<?php echo $_REQUEST['page']; ?>&limit=<?php echo $_REQUEST['limit']; ?>" title="Add New Item"><img src="images/new_f2.png" border="0" alt="Add" /></a></td></tr></table>
<div>&nbsp;</div>
</div>
<div id="editcell">
	...
	<thead>
		<tr>
			<th>#</th>
			<th class="title" style="text-align:left;">Ad Category</th>
			<th class="title" style="text-align:left;">Ad Code</th>
	...
	<?php } ?>
		...
		<tbody>
			...
			<tr class="row<?php echo ($cnt%2); ?>">
				...
				<td><?php echo fetchName('adcat', $item->cid); ?></td>
				<td><?php echo nl2br(safeHTML($item->ad_code)); ?></td>
				<td align="center">
...
function add_edit() {
	...
	if($_REQUEST['action'] == 'edit') {
		$database =& JFactory::getDBO();
		$database->setQuery("select * from `#__".RSWEBSOLS_TABLE_PREFIX."_ad` where `id`='".$_REQUEST['id']."'");
...
<div>
<div>&nbsp;</div>
<div><h1><?php echo ucfirst(strtolower($_REQUEST['action'])); ?> Ad</h1></div>
<div>&nbsp;</div>
</div>
<fieldset class="adminform">
				...
				function submitFormRS() {
					...
					else if(f.ad_code.value == '') {
						alert("Please enter ad code.");
						f.ad_code.focus();
						return false;
					}
					else {
				...
				<input type="hidden" name="limit" id="limit" value="<?php echo $_REQUEST['limit']; ?>" />
					...
					<tr>
						<td class="key">Ad Category:</td>
						<td colspan="2">
						<?php $adcats = fetchAdCategories(); ?>
						<select name="cid">
						<?php foreach($adcats as $adcat) { ?>
						<option value="<?php echo $adcat->id; ?>" <?php if($row->cid == $adcat->id) { ?>selected="selected"<?php } ?>><?php echo $adcat->name; ?></option>
						<?php } ?>
						</select>
						</td>
					</tr>
					<tr>
						<td class="key">Ad Code:</td>
						<td colspan="2">
						<textarea name="ad_code" style="width:300px; height:200px;"><?php echo safeHTML($row->ad_code); ?></textarea>
						</td>
					</tr>
					<tr>
...
function save() {
	...
	$database =& JFactory::getDBO();
	if($_POST['id']>0) {
		$database->setQuery("update `#__".RSWEBSOLS_TABLE_PREFIX."_ad` set `cid`='".$_POST['cid']."', `ad_code`='".$_POST['ad_code']."', `status`='".$_POST['published']."' where `id`='".$_POST['id']."'");
	}
	else {
		$database->setQuery("insert into `#__".RSWEBSOLS_TABLE_PREFIX."_ad`(`id`, `cid`, `ad_code`, `status`) values('', '".$_POST['cid']."', '".$_POST['ad_code']."', '".$_POST['published']."')");
	}
	$database->query();
...
function publish_unpublish() {
	$database =& JFactory::getDBO();
	$database->setQuery("update `#__".RSWEBSOLS_TABLE_PREFIX."_ad` set `status`=if((`status` > 0), 0, 1) where `id`='".$_REQUEST['id']."'");
	$database->query();
	header("location:index.php?option=".$_REQUEST['option']."&task=".$_REQUEST['task']."&page=".$_REQUEST['page']."&limit=".$_REQUEST['limit']."&result=true");
	exit();
}
...
function delete() {
	$database =& JFactory::getDBO();
	$database->setQuery("delete from `#__".RSWEBSOLS_TABLE_PREFIX."_ad` where `id`='".$_REQUEST['id']."'");
	$database->query();
	header("location:index.php?option=".$_REQUEST['option']."&task=".$_REQUEST['task']."&page=".$_REQUEST['page']."&limit=".$_REQUEST['limit']."&result=true");
	exit();
}
